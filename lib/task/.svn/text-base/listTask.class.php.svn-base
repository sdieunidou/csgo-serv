...
class listTask extends sfBaseTask
{
  private
    $counter = 1,
    $indicators = array('-', '\\', '|', '/');
  ...
  protected function configure()
  {
    $this->addArguments(array(
      new sfCommandArgument('game', sfCommandArgument::REQUIRED, 'Game'),
    ));
...
EOF;
  ...
  protected function execute($arguments = array(), $options = array())
  {
    ...
    if(!$Game = GameTable::getInstance()->findOneByClass($arguments['game']))
    {
      throw new Exception(sprintf('Game object with class %s not exist', $arguments['game']));
    }

    $Master = new Master($Game->getClass());
    ...
    do
    {
      if($try > 0)
      {
        $this->logSection('master', sprintf('Connexion avec le master stoppé, %d essais restants.', (3 - $try)));
      }

      $list_server = $Master->retrieveList();
      $try++;
    } while($list_server == false && $try < 3);

    $count_new = 0;
    $count = count($list_server);
    
    foreach($list_server as $key => $server2)
    {
      $server = explode(':', $server2);
      $server['ip'] = $server[0];
      $server['port'] = $server[1];

      if($server['ip'] == '0.0.0.0' ||$server['ip'] == '255.255.255.255' ||empty($server['ip']))
      {
        $count--;
        continue;
      }
      
      $check_if_exist = ServerTable::getInstance()->countByIpAndPort($server['ip'], $server['port']);
      if(!$check_if_exist)
      {
        $Server = new Server();
        $Server->ip = $server['ip'];
        $Server->port = $server['port'];
        $Server->last_online = date('Y-m-d H:i:s' ,time());
        $Server->game_id = $Game->getId();
        $Server->save();
        $Server->free(true);

        $count_new++;
      }
      $this->progressBar($key, ($count - 1));
    }

    $this->logSection('master', sprintf('%d serveurs reçus, dont %d nouveaux', count($list_server), $count_new));
  }
